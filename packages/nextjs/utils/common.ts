...
import { AbiComponent, AbiItem, ContractAbi } from "../types/types";
import { graphqlEndpoint } from "./query";
import request from "graphql-request";
import { TransactionReceipt, decodeEventLog, formatUnits, keccak256, stringToBytes } from "viem";
...
export const pollUntilMetadataIsAvailable = async (pointer: string): Promise<boolean> => {
  ...
  const fetchMetadata: any = async () => {
    ...
    if (metadata) {
      ...
      return await new Promise(resolve => setTimeout(resolve, 2000)).then(fetchMetadata);
    }
  };
  return await fetchMetadata();
};

export const pollUntilDataIsIndexed = async (QUERY_ENDPOINT: any, data: any, propToCheck: string): Promise<boolean> => {
  let counter = 0;
  const fetchData: any = async () => {
    ...
    if (response && response[propToCheck] !== null) {
      ...
      return await new Promise(resolve => setTimeout(resolve, 2000)).then(fetchData);
    }
  };
...
export const NATIVE = "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE".toLowerCase();
...
export const getEventValues = (receipt: TransactionReceipt, abi: ContractAbi, eventName: string): any => {
  const { logs } = receipt;
  const event = abi.filter(item => item.type === "event" && item.name === eventName)[0];

  console.log("event", event);

  const eventTopic = getEventTopic(event);

  const log = logs.find(log => log.topics[0]?.toLowerCase() === eventTopic.toLowerCase());
