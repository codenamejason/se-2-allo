...
import { getMicroGrantRecipientQuery, getProfile, getProfilesByOwnerQuery, graphqlEndpoint } from "./query";
...
const getProfilesByOwner = async ({
  ...
  const filteredProfiles = response.profilesByOwner.filter(
    profile => new Date(profile.createdAt) > new Date("2023-11-02T00:00:00+00:00"),
  );
...
export const getApplicationData = async (
  ...
  try {
    ...
    const metadata: TApplicationMetadata = await ipfsClient.fetchJson(application.metadataPointer);
