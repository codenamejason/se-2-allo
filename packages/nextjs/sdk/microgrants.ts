...
export const deployMicrograntsStrategy = async (pointer: any, profileId: string) => {
...
export const createApplication = async (data: TNewApplication, chain: number, poolId: number): Promise<string> => {
  ...
  // 5. Index Metadata
  const pollingMetadataResult = await pollUntilMetadataIsAvailable(pointer.IpfsHash);
...
export const allocate = async (data: Allocation) => {
  ...
  if (strategy) {
    ...
    const txData: TransactionData = strategy.getAllocationData(data.recipientId, data.status);
